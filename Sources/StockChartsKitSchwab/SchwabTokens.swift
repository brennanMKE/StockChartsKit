...
/// The persisted OAuth 2.0 token bundle for a Schwab connection.
///
/// Stored as a single JSON item in the ``TokenStore`` keyed by
/// `"schwab.<connectionID>"`. Never logged.
...
/// through the OAuth 2.0 token endpoint when expired.
///
...
///
/// Schwab's token endpoint authenticates the *client* with HTTP Basic auth
/// (`Authorization: Basic base64(client_id:client_secret)`), so this provider
/// builds that header for both the authorization-code exchange and the refresh.
actor SchwabTokenProvider {
  ...
  private func refresh(using refreshToken: String) async throws -> String {
    ...
    let bundle = TokenBundle(
      ...
      // Schwab may rotate the refresh token; keep the prior one if omitted.
  ...
  private func postForm(_ fields: [String: String]) async throws -> TokenResponse {
    ...
    let client = HTTPClient(providerID: ProviderID(rawValue: "schwab"), session: session, now: now)
